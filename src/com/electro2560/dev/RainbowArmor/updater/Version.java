...
public class Version implements Comparable<Version>{
	...
	public Version(String value){
		if (value == null) throw new IllegalArgumentException("Version can not be null");
		if (!value.matches("^[0-9]+(\\.[0-9]+)*$")) throw new IllegalArgumentException("Invalid version format");
   
		String[] split = value.split("\\.");
		
		parts = new int[split.length];
		for (int i = 0; i < split.length; i++) parts[i] = Integer.parseInt(split[i]);
	}
	...
	public static boolean equals(Version verA, Version verB){
 	...
 	public String toString(){
 		...
 		for (int i = 0; i < parts.length; i++) split[i] = String.valueOf(parts[i]);
